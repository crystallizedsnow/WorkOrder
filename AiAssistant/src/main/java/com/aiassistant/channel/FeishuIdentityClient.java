...
public class FeishuIdentityClient {
    ...
    @Value("${workorder.channel.service-key}") private String serviceKey;
    ...
    public ProxyToken exchange(String tenantKey, String unionId, String openId, String channelSessionId) {
        ...
        return new ProxyToken(data.path("accessToken").asText(), Instant.parse(data.path("expiresAt").asText()), data.path("userId").asText());
    }
    ...
    private JsonNode call(String path, Object body) {
        ...
        try {
            ...
            if (root.path("code").asInt() != 1) throw new IllegalStateException(root.path("msg").asText("Channel identity request failed"));
            return root.path("data");
        } catch (java.io.IOException ex) { throw new IllegalStateException("Invalid backend response", ex); }
    }
